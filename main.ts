...
import PublicationManager from 'src/PublicationManager';
...
export default class DigitalGarden extends Plugin {
	...
	async addCommands() {
		...
		this.addCommand({
			id: 'dg-open-publication-manager',
			name: 'Open Publication Manager (Beta)',
			callback: async () => {
				const publishManager = new PublicationManager(this.app, this.settings);
				await publishManager.expandFileSystemTree(new Modal(this.app), "changedFiles");
			}
		});
